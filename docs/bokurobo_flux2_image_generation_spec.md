...
ファイル: `workflows/flux2_klein_bokurobo_beforeafter_v2.json`
...
- [ ] `StringConcatenate`（ユーザー入力・変形前）の`string_a`に、変形前オブジェクトの日本語プロンプトを貼り付け
- [ ] `StringConcatenate`（ユーザー入力・変形後）の`string_a`に、変形後ロボの日本語プロンプトを貼り付け（背景・カメラ記述は変形前と同一文言にする）。**`string_b`（共通スタイル文言）は編集しない**
...
## 9. 共通ビジュアル方針：戦隊ロボ風デザイン（2026-07-15決定）

bokurobo全エピソード共通のビジュアル方針として、「変形後」ロボの見た目に汎用的な戦隊ヒーローロボ風デザイン要素を採用する。特定作品・ロボ名は使用しない（一般名詞での記述に限定）。

### 適用範囲
- 「変形後」プロンプトにのみ適用。「変形前」（元オブジェクト）には適用しない
- 全エピソード共通（エピソードごとに再検討しない）

### 実装方法
`flux2_klein_bokurobo_beforeafter_v2.json` の「ユーザー入力（日本語・変形後）」ノード（`StringConcatenate`）で対応：
- `string_a`：エピソードごとのお題プロンプト（毎回編集）
- `string_b`：共通スタイル文言（固定・触らない）

固定文言（`string_b`）：
```
顔はバイザー型で中央に一本の発光ライン、胸部には変形元のパーツを活かしたエンブレム状の意匠、
輪郭の縁にはクロムメッキのトリム、原色（赤・青・黄のいずれか）を差し色として部分的に使用、
重心をやや低くし胸を張ったヒロイックな直立ポーズ。
全身を単色で塗り潰さず、変形元オブジェクトの質感・色を基調として残すこと。
```

### 採用理由（案2を選定）
「全身を原色で塗り分ける」フル戦隊風（案1）は部位対応（画面→胸、スタンド→脚等）の視認性を下げるため不採用。「顔とポーズのみ変更」（案3）は戦隊感が弱いため不採用。元オブジェクトの質感を残しつつバイザー顔・エンブレム・クロムトリムをアクセントとして加える案2を採用。

### 変更履歴
- 2026-07-15: v1→v2。共通スタイル文言をワークフローに固定組み込み
