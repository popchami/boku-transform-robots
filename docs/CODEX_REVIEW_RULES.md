...
| 2026-07-14 | `render`サブコマンドの最小設計・実装レビュー | シーン単位で正規化してconcatする方針を承認。字幕/fontのFFmpegフィルタ解析問題はtextfile化と一時ディレクトリ内へのfontコピーで解消 | `src/bokurobo/render.py`・CLI・テストを実装。83 tests PASS、`git diff --check` OK、実ffmpeg/ffprobeによる5シーンrender成功 |
...
- **パイプライン分割**: `validate`（マニフェスト検証）→`render`（動画合成）の2段構成。両サブコマンドとも実装済み
...
- **変形シーンの表現方針を変更**: 当初「いきなり重い動画生成AIは使わない」方針だったが、ユーザーから「変形は動画で見せたい（高品質にしたい）」というフィードバックがあり方針転換。**「変形」シーンのみ**text-to-video系のAI動画生成を使う（2026-07-14中にimage-to-videoから変更。採用検討中のLoRAがtext-to-video専用のため）。他4シーン（お題提示・変形前・変形後・オチ）は引き続き静止画＋ズーム/切り替え
...
- 具体的なtext-to-videoモデル・実行環境・VRAM要件・生成時間・コスト試算は**未検証**。本実装前に調査とCodexレビューが必要（詳細は`docs/SPEC.md`「変形シーンのAI動画生成」、および別リポジトリ`comfyui-video-system`のADR 0001参照）
